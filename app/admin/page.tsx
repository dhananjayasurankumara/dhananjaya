...
function DashboardTab({ onNavigate }: { onNavigate: (tab: string) => void }) {
    ...
    useEffect(() => {
        ...
        ]).then(([prods, usrs]) => setStats({ products: prods.length, users: usrs.length, orders: 0 }));
    }, []);

    const quickActions = [
        { label: 'Edit Hero', tab: 'hero', icon: '◉', desc: 'Update headline & animation' },
        { label: 'Add Product', tab: 'shop', icon: '⬡', desc: 'Add items to your store' },
        { label: 'Edit Content', tab: 'content', icon: '✦', desc: 'Update portfolio sections' },
        { label: 'Manage Users', tab: 'users', icon: '◇', desc: 'View registered accounts' },
        { label: 'Site Settings', tab: 'settings', icon: '⚙', desc: 'Logo, colors, meta' },
    ];
    ...
    return (
        <div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(180px, 1fr))', gap: '1rem', marginBottom: '2rem' }}>
                {[
                    { label: 'Products', value: stats.products, icon: '⬡', color: '#818cf8' },
                    { label: 'Users', value: stats.users, icon: '◇', color: '#34d399' },
                    { label: 'Orders', value: stats.orders, icon: '◈', color: '#fb923c' },
                ].map(s => (
                    <div key={s.label} style={{ ...S.card, position: 'relative', overflow: 'hidden' }}>
                        ...
                        <div style={{ fontSize: '2.2rem', fontWeight: 800, color: '#fff', lineHeight: 1 }}>{s.value}</div>
                        <div style={{ fontSize: '0.6rem', letterSpacing: '0.2em', textTransform: 'uppercase', color: 'rgba(255,255,255,0.35)', marginTop: '0.4rem' }}>{s.label}</div>
                    </div>
                ))}
            </div>
            ...
            <div style={{ marginBottom: '1rem' }}>
                <div style={{ fontSize: '0.6rem', letterSpacing: '0.2em', textTransform: 'uppercase', color: 'rgba(255,255,255,0.3)', marginBottom: '1rem' }}>Quick Actions</div>
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: '0.75rem' }}>
                    {quickActions.map(a => (
                        <button key={a.tab} onClick={() => onNavigate(a.tab)} style={{ ...S.card, background: 'rgba(255,255,255,0.02)', cursor: 'pointer', textAlign: 'left', border: '1px solid rgba(255,255,255,0.06)', borderRadius: '12px', padding: '1.25rem', transition: 'all 0.2s', fontFamily: 'inherit', display: 'block', width: '100%' }}>
                            <div style={{ fontSize: '1rem', marginBottom: '0.6rem', opacity: 0.7 }}>{a.icon}</div>
                            <div style={{ fontSize: '0.8rem', fontWeight: 700, color: '#fff', marginBottom: '0.3rem' }}>{a.label}</div>
                            <div style={{ fontSize: '0.65rem', color: 'rgba(255,255,255,0.3)', lineHeight: 1.4 }}>{a.desc}</div>
                        </button>
                    ))}
                </div>
            </div>
